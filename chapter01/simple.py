...
"""
Name：Cookie的名称，Cookie 一旦创建，名称便不可更改
Value：Cookie的值，如果值为 Unicode 字符，需要为字符编码。如果值为二进制数据，则需要使用 BASE64 编码

Max Age：Cookie 失效的时间，单位秒，常和 Expires 一起使用，通过它可以计算出有效时间。Max Age 如果为正数，则 Cookie 在 Max Age 秒之后失效。如果为负数，则关闭浏览器时 Cookie 就失效，浏览器也不会以任何形式保存该 Cookie 
...
Secure：Cookie 是否仅被使用安全协议传输。安全协议有 HTTPS、SSL 等，在网络上传输数据之前会先将数据加密，默认值为 false
"""
...
SOCKS 代理，只是单纯传递数据包，不关心具体协议和用法，所以速度快很多，一般有缓存功能，端口一般为 1080。SOCKS 代理又分为 SOCKS4 和 SOCKS5，SOCKS4 只支持 TCP，而 SOCKS5 支持 TCP 和 UDP，以及各种身份验证机制、服务器端域名解析等
"""

"""
高度匿名代理，高度匿名代理会将数据包原封不动的转发，在服务端看来就好像真的是一个普通客户端在访问，而记录的 IP 是代理服务器的 IP
普通匿名代理，普通匿名代理会在数据包上做一些改动，服务端可能会发现这是个代理服务器，也有一定概率追查到客户端的真实 IP，代理服务器通常会加入的 HTTP 头有 HTTP_VIA、HTTP_X_FORWARDED_FOR
透明代理，透明代理不但改动了数据包，还会告诉服务器客户端的真实 IP。这种代理除了能用缓存技术提高浏览速度，能用内容过滤提高安全性之外，并无其他显著作用，最常见的例子是内网中的硬件防火墙
间谍代理，由组织或个人创建的代理服务器，用于记录用户传输的数据，然后对数据进行研究、监控等
"""
