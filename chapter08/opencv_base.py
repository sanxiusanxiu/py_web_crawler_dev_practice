...
def get_gaussian_blur_image(image):
    """
    OpenCV 中用于实现高斯模糊的方法，叫作 GaussianBlur，其声明如下：
    def GaussianBlur(src, ksize, sigmaX, dst=None, sigmaY=None, borderType=None)
    src：需要处理的图片
    ksize：高斯滤波处理所用的高斯内核大小，需要传人一个元组，包含 x 和 y 两个元素
    sigmaX：高斯内核函数在 X 方向上的标准偏差
    sigmaY：高斯内核函数在 Y 方向上的标准偏差。若 sigmaY 为 0，就将它设为 sigmaX；若 sigmaX 和 sigmaY 都是 0，就通过 ksize 计算出 sigmaX 和 sigmaY

    这里 ksize 和 sigmaX 是必传参数
    """
...
def get_canny_image(image):
    """
    OpenCV 中用于实现边缘检测的方法，叫作 Canny，其声明如下：
    ...
    通常来说，只需要设置 threshold1 和 threshold2 的值即可，其数值大小需要视具体图片而定
    """
...
def get_contours(image):
    """
    OpenCV 中提取边缘轮廓的方法叫作 findContours，其声明如下：
    def findContours(image, mode, method, contours=None, hierarchy=None, offset=None)
    image：需要处理的图片
    mode：用于定义轮廓的检索模式
    method：用于定义轮廓的近似方法

    这里我们将 mode 设置为 RETRCCOMP，将 method 设置为 CHAINAPPROXSIMPLE，具体的选择标准可以参考 OpenCV 官方文档的介绍
    """
...
经过实际测量可以得出目标缺口的外接矩形的高度大约是验证码高度的 0.25 倍，宽度大约是验证码宽度的 0.15 倍，
所以在允许误差是 20% 的情况下，可以根据验证码的宽、高信息大约计算出外接矩形的面积和周长的取值范围，同时，
缺口位置（缺口左侧）有一个最小偏移量和一个最大偏移量，这里的最小偏移量是验证码宽度的 0.2 倍，最大偏移量
是验证码宽度的 0.85 倍，将这些内容综合起来，我们可以定义以下 3 个阈值方法
"""

# 定义目标轮廓的面积下限和面积上限，分别为 contour_area_min 和 contour_area_max
...
# 定义目标轮廓的周长下限和周长上限，分别为 arc_length_min 和 arc_length_max
...
# 定义缺口位置的偏移量下限和偏移量上限，分别为 offset_min 和 offset_max
...
def main():
    # 设置待识别的图片
    image_raw = cv2.imread('opencv_base.png')
    ...
    # 保存图片
    cv2.imwrite('opencv_base_image_canny.png', image_canny)
    cv2.imwrite('opencv_base_image_gaussian_blur.png', image_gaussian_blur)
    ...
    for contour in contours:
        ...
        if contour_area_min < cv2.contourArea(contour) < contour_area_max and \
            ...
            offset = x
    cv2.imwrite('opencv_base_image_label.png', image_raw)
    print('offset:', offset)
